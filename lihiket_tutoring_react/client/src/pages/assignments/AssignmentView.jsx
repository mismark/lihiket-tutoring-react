...
function GradePanel({ sub, totalMarks, onGraded, theme }) {
  ...
  const inputCls = `w-full px-3 py-2 rounded-xl border text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 ${dark ? 'bg-slate-900 border-slate-600 text-white' : 'bg-gray-50 border-gray-300 text-gray-900'}`;
  ...
  return (
    <div className={`p-4 rounded-xl border space-y-3 ${dark ? 'border-slate-700 bg-slate-700/30' : 'border-gray-200 bg-gray-50'}`}>
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className={`block text-xs font-semibold mb-1 ${dark ? 'text-slate-400' : 'text-gray-500'}`}>Marks (/ {totalMarks})</label>
          <input type="number" value={marks} onChange={e => setMarks(e.target.value)} min="0" max={totalMarks} className={inputCls} />
        </div>
        <div>
          <label className={`block text-xs font-semibold mb-1 ${dark ? 'text-slate-400' : 'text-gray-500'}`}>Feedback</label>
          <input value={feedback} onChange={e => setFeedback(e.target.value)} placeholder="Optional…" className={inputCls} />
        </div>
      </div>
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className={`flex flex-col w-full max-w-xl max-h-[92vh] rounded-2xl border shadow-2xl ${dark ? 'bg-slate-800 border-slate-700' : 'bg-white border-gray-200'}`}>
        ...
        <div className={`flex items-start justify-between px-6 py-4 border-b flex-shrink-0 ${dark ? 'border-slate-700' : 'border-gray-200'}`}>
          <div>
            <h2 className={`text-base font-bold ${dark ? 'text-white' : 'text-gray-900'}`}>{a.title}</h2>
        ...
        <div className="overflow-y-auto flex-1 px-6 py-5 space-y-5">
          ...
          <div className="grid grid-cols-2 gap-3">
            {[
              ...
              <div key={label} className={`p-3 rounded-xl ${dark ? 'bg-slate-700/50' : 'bg-gray-50'}`}>
                ...
                <p className={`text-sm font-semibold ${dark ? 'text-white' : 'text-gray-900'}`}>{String(value)}</p>
              </div>
            ))}
          </div>

          {a.subject && <p className={`flex items-center gap-2 text-sm ${dark ? 'text-slate-300' : 'text-gray-700'}`}><FiBook className="w-4 h-4 text-blue-500" />{a.subject.name} · {a.subject.gradeLevel}</p>}
          {a.description && <p className={`text-sm ${dark ? 'text-slate-300' : 'text-gray-700'}`}>{a.description}</p>}
          {a.instructions && (
            <div>
              <p className={`text-xs font-bold uppercase tracking-wider mb-1.5 ${dark ? 'text-slate-400' : 'text-gray-500'}`}>Instructions</p>
              <p className={`text-sm leading-relaxed whitespace-pre-line ${dark ? 'text-slate-300' : 'text-gray-700'}`}>{a.instructions}</p>
            </div>
          )}
          {a.attachmentUrl && (
            <a href={`${SERVER}${a.attachmentUrl}`} target="_blank" rel="noopener noreferrer"
              className={`inline-flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-semibold transition ${dark ? 'bg-slate-700 text-slate-200 hover:bg-slate-600' : 'bg-gray-100 text-gray-800 hover:bg-gray-200'}`}>
          ...
          {sub && (
            <div className={`p-4 rounded-xl border ${sub.status === 'graded' ? 'border-emerald-400/40 bg-emerald-50 dark:bg-emerald-500/10' : 'border-blue-400/40 bg-blue-50 dark:bg-blue-500/10'}`}>
              <p className={`text-xs font-bold uppercase tracking-wider mb-2 ${dark ? 'text-slate-400' : 'text-gray-500'}`}>My Submission</p>
              {sub.text && <p className={`text-sm mb-2 ${dark ? 'text-slate-300' : 'text-gray-700'}`}>{sub.text}</p>}
              ...
              <div className="flex justify-between items-center">
                <span className={`text-xs ${dark ? 'text-slate-400' : 'text-gray-500'}`}>{fmt(sub.submittedAt)}{sub.late ? ' · Late' : ''}</span>
                {sub.marks != null && <span className={`text-sm font-extrabold ${dark ? 'text-white' : 'text-gray-900'}`}>{sub.marks} / {a.totalMarks}</span>}
              </div>
              {sub.feedback && <p className={`text-xs mt-1 italic ${dark ? 'text-slate-400' : 'text-gray-500'}`}>{sub.feedback}</p>}
            </div>
          )}
          ...
          {canManage && (
            <div>
              ...
              {showSubs && subs.map(s => (
                <div key={s._id} className={`rounded-xl border p-4 mb-3 ${dark ? 'border-slate-700 bg-slate-700/30' : 'border-gray-200'}`}>
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <div>
                      <p className={`text-sm font-semibold ${dark ? 'text-white' : 'text-gray-900'}`}>{s.student?.firstName} {s.student?.lastName}</p>
                      <p className={`text-xs ${dark ? 'text-slate-400' : 'text-gray-500'}`}>{fmt(s.submittedAt)}{s.late ? ' · Late' : ''}</p>
                    </div>
                    {s.marks != null && <span className={`ml-auto text-sm font-bold ${dark ? 'text-white' : 'text-gray-900'}`}>{s.marks}/{a.totalMarks}</span>}
                  </div>
                  {s.text && <p className={`text-xs mb-2 ${dark ? 'text-slate-400' : 'text-gray-600'}`}>{s.text}</p>}
