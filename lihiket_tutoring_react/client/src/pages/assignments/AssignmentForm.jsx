...
export default function AssignmentForm({ title, initial, subjects, onSubmit, onCancel, saving, theme }) {
  ...
  const inputCls = `w-full px-3 py-2.5 rounded-xl border text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 ${
    dark ? 'bg-slate-900 border-slate-600 text-white placeholder-slate-500' : 'bg-slate-50 border-gray-300 text-slate-900'
  }`;
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className={`flex flex-col w-full max-w-xl max-h-[94vh] rounded-2xl border shadow-2xl ${dark ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'}`}>
        <div className={`flex items-center justify-between px-6 py-4 border-b flex-shrink-0 ${dark ? 'border-slate-700' : 'border-slate-200'}`}>
          ...
          <button type="button" onClick={onCancel} className={`p-2 rounded-lg ${dark ? 'hover:bg-slate-700 text-slate-400' : 'hover:bg-slate-100 text-slate-500'}`}><FiX className="w-5 h-5" /></button>
        </div>
          ...
          <div className="overflow-y-auto flex-1 px-6 py-5 space-y-4">
            ...
            <div className={`flex items-center justify-between p-3 rounded-xl border ${dark ? 'border-slate-600 bg-slate-900/50' : 'border-slate-200 bg-slate-50'}`}>
              <div>
                ...
                <p className={`text-xs ${dark ? 'text-slate-500' : 'text-slate-400'}`}>Students can submit after the due date</p>
              </div>
              <label className="relative inline-flex items-center cursor-pointer">
                <input type="checkbox" name="allowLate" checked={form.allowLate} onChange={handleChange} className="sr-only peer" />
                <div className="w-10 h-6 bg-slate-200 rounded-full peer peer-checked:bg-amber-500 dark:bg-slate-600 dark:peer-checked:bg-amber-500 after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:after:translate-x-4" />
              </label>
            </div>

            {/* Attachment upload */}
            <div>
              <label className={lbl}>Attachment <span className={`font-normal ${dark ? 'text-slate-500' : 'text-slate-400'}`}>(optional)</span></label>
              <div onClick={() => fileInputRef.current?.click()}
                ...
                <span className={`text-sm flex-1 truncate ${dark ? 'text-slate-400' : 'text-slate-500'}`}>
              ...
              {initial && !file && initial.attachmentUrl && (
                <p className={`mt-1 text-xs ${dark ? 'text-slate-500' : 'text-slate-400'}`}>Current: <a href={`${SERVER}${initial.attachmentUrl}`} target="_blank" rel="noopener noreferrer" className="text-blue-500 hover:underline">{initial.attachmentName || 'file'}</a></p>
              )}
            </div>
          </div>

          <div className={`flex gap-3 px-6 py-4 border-t flex-shrink-0 ${dark ? 'border-slate-700' : 'border-slate-200'}`}>
            <button type="button" onClick={onCancel} className={`flex-1 py-2.5 rounded-xl font-semibold text-sm transition ${dark ? 'bg-slate-700 text-slate-300 hover:bg-slate-600' : 'bg-slate-100 text-slate-700 hover:bg-slate-200'}`}>Cancel</button>
            <button type="submit" disabled={saving} className="flex-1 py-2.5 rounded-xl font-semibold text-sm bg-blue-600 hover:bg-blue-700 text-white transition disabled:opacity-50 flex items-center justify-center gap-2">
              {saving ? <><div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" /> Savingâ€¦</> : <><FiSave className="w-4 h-4" /> Save</>}
            </button>
          </div>
        </form>
      </div>
