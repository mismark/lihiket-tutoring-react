...
export default function QuestionDelete({ question, onClose, onDeleted, theme }) {
      ...
      <div className={`w-full max-w-md rounded-2xl border shadow-2xl ${
        ...
        <div className={`flex items-center justify-between px-6 py-4 border-b ${dark ? 'border-slate-700' : 'border-gray-200'}`}>
          <div className="flex items-center gap-2 text-red-500">
            ...
            <h2 className={`text-lg font-bold ${dark ? 'text-white' : 'text-gray-900'}`}>Delete Question</h2>
          </div>
          ...
          <div className={`flex gap-3 p-4 rounded-xl border ${dark ? 'bg-red-500/10 border-red-500/20' : 'bg-red-50 border-red-200'}`}>
            ...
            <div>
              ...
              <p className={`text-xs mt-1 ${dark ? 'text-slate-400' : 'text-gray-600'}`}>
                The question will be permanently removed from the question bank and any exams or quizzes using it.
              </p>
            </div>
          </div>

          <div className={`p-3 rounded-xl ${dark ? 'bg-slate-700/50' : 'bg-gray-50'}`}>
            <p className={`text-xs font-semibold mb-1 ${dark ? 'text-slate-400' : 'text-gray-500'}`}>Question</p>
            <p className={`text-sm line-clamp-2 ${dark ? 'text-white' : 'text-gray-900'}`}>{question.text}</p>
          </div>
