  ...
  return (
    <div className={`min-h-screen p-4 md:p-8 ${dark ? 'bg-slate-900' : 'bg-slate-50'}`}>
      <div className="max-w-7xl mx-auto space-y-5">
        ...
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div>
            ...
            <p className={`text-sm mt-0.5 ${dark ? 'text-slate-400' : 'text-slate-500'}`}>
              {loading ? 'â€¦' : `${total} question${total !== 1 ? 's' : ''}`}
            </p>
          </div>
          {canManage && (
            <button
        ...
        <div className={`rounded-2xl border shadow-sm p-4 space-y-3 ${dark ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'}`}>
        ...
        {loading ? (
          <div className="text-center py-16">
            ...
            <p className={`mt-4 text-sm ${dark ? 'text-slate-400' : 'text-slate-500'}`}>Loading questionsâ€¦</p>
          </div>
        ) : questions.length === 0 ? (
          <div className={`rounded-2xl border p-12 text-center ${dark ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'}`}>
            <FiBookOpen className={`w-12 h-12 mx-auto mb-4 ${dark ? 'text-slate-600' : 'text-slate-300'}`} />
