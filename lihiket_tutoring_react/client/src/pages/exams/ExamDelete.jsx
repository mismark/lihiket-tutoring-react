...
export default function ExamDelete({ exam, onClose, onDeleted, theme }) {
  ...
  const handleDelete = async () => {
    setLoading(true);
    try { await deleteExam(exam._id); toast.success('Exam deleted'); onDeleted(); onClose(); }
    catch (err) { toast.error(err.message || 'Failed to delete exam'); }
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className={`w-full max-w-md rounded-2xl border shadow-2xl ${dark ? 'bg-slate-800 border-slate-700' : 'bg-white border-gray-200'}`}>
        <div className={`flex items-center justify-between px-6 py-4 border-b ${dark ? 'border-slate-700' : 'border-gray-200'}`}>
          <div className="flex items-center gap-2 text-red-500"><FiTrash2 className="w-5 h-5" /><h2 className={`text-lg font-bold ${dark ? 'text-white' : 'text-gray-900'}`}>Delete Exam</h2></div>
        ...
        <div className="p-6 space-y-4">
          <div className={`flex gap-3 p-4 rounded-xl border ${dark ? 'bg-red-500/10 border-red-500/20' : 'bg-red-50 border-red-200'}`}>
            ...
            <p className={`text-sm ${dark ? 'text-slate-300' : 'text-gray-700'}`}>Permanently delete <strong>"{exam.title}"</strong> and all student results?</p>
          </div>
          <div className="flex gap-3">
            <button onClick={onClose} disabled={loading} className={`flex-1 py-2.5 rounded-xl font-semibold text-sm ${dark ? 'bg-slate-700 text-slate-300 hover:bg-slate-600' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'} disabled:opacity-50 transition`}>Cancel</button>
