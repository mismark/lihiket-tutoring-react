...
export default function QuizDelete({ quiz, onClose, onDeleted, theme }) {
  ...
  const handleDelete = async () => {
    setLoading(true);
    try { await deleteQuiz(quiz._id); toast.success('Quiz deleted'); onDeleted(); onClose(); }
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className={`w-full max-w-md rounded-2xl border shadow-2xl ${dark ? 'bg-slate-800 border-slate-700' : 'bg-white border-gray-200'}`}>
        <div className={`flex items-center justify-between px-6 py-4 border-b ${dark ? 'border-slate-700' : 'border-gray-200'}`}>
          <div className="flex items-center gap-2 text-red-500"><FiTrash2 className="w-5 h-5" /><h2 className={`text-lg font-bold ${dark ? 'text-white' : 'text-gray-900'}`}>Delete Quiz</h2></div>
        ...
        <div className="p-6 space-y-4">
          <div className={`flex gap-3 p-4 rounded-xl border ${dark ? 'bg-red-500/10 border-red-500/20' : 'bg-red-50 border-red-200'}`}>
            ...
            <p className={`text-sm ${dark ? 'text-slate-300' : 'text-gray-700'}`}>Permanently delete <strong>"{quiz.title}"</strong> and all student results?</p>
          </div>
