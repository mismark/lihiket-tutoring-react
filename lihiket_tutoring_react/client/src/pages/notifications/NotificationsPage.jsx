        ...
        <div className="flex items-center justify-between flex-wrap gap-3">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              title="Refresh"
              className={`p-2 rounded-xl border transition ${dark ? 'bg-slate-800 border-slate-700 text-slate-400 hover:bg-slate-700' : 'bg-white border-gray-200 text-gray-500 hover:bg-gray-100'}`}
            >
            ...
            {unreadCount > 0 && (
              <button
                ...
                className={`flex items-center gap-1.5 px-3 py-2 rounded-xl border text-xs font-semibold transition ${dark ? 'bg-slate-800 border-slate-700 text-emerald-400 hover:bg-slate-700' : 'bg-white border-gray-200 text-emerald-600 hover:bg-emerald-50'}`}
              >
            ...
            {total > 0 && (
              <button
                ...
                className={`flex items-center gap-1.5 px-3 py-2 rounded-xl border text-xs font-semibold transition ${dark ? 'bg-slate-800 border-slate-700 text-red-400 hover:bg-red-500/10' : 'bg-white border-gray-200 text-red-500 hover:bg-red-50'}`}
              >
        ...
        <div className={`rounded-2xl border overflow-hidden shadow-sm ${dark ? 'bg-slate-800 border-slate-700' : 'bg-white border-gray-200'}`}>
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-14 gap-3">
              <FiInbox className={`w-12 h-12 ${dark ? 'text-slate-600' : 'text-gray-300'}`} />
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-center gap-2">
            <button
              ...
              className={`px-4 py-2 rounded-xl text-sm font-semibold transition disabled:opacity-40 ${dark ? 'bg-slate-800 border border-slate-700 text-slate-300 hover:bg-slate-700' : 'bg-white border border-gray-200 text-gray-700 hover:bg-gray-100'}`}
            >
              ← Prev
            </button>
