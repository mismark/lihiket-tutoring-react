...
// ─── File filters ─────────────────────────────────────────────────────────────
const imageFilter    = (_req, file, cb) => cb(null, /jpeg|jpg|png|webp/.test(path.extname(file.originalname).toLowerCase()));
const documentFilter = (_req, file, cb) => cb(null, /pdf|doc|docx|ppt|pptx|xls|xlsx|txt|zip/.test(path.extname(file.originalname).toLowerCase()));
const videoFilter    = (_req, file, cb) => cb(null, /mp4|webm|mov|avi/.test(path.extname(file.originalname).toLowerCase()));
...
// ─── Combined arrays ──────────────────────────────────────────────────────────
const uploadCVCloud         = [uploadCV,         pushToCloudinary('lihiket/cvs',         'raw')];
const uploadAvatarCloud     = [uploadAvatar,      pushToCloudinary('lihiket/avatars',      'image')];
const uploadDocumentCloud   = [uploadDocument,    pushToCloudinary('lihiket/documents',    'raw')];
const uploadLessonCloud     = [uploadLesson,      pushToCloudinary('lihiket/lessons',      'video')];
const uploadAssignmentCloud = [uploadAssignment,  pushToCloudinary('lihiket/assignments',  'raw')];

module.exports = {
  uploadAvatar, uploadDocument, uploadLesson, uploadCV, uploadAssignment,
  uploadCVCloud, uploadAvatarCloud, uploadDocumentCloud, uploadLessonCloud, uploadAssignmentCloud,
};
